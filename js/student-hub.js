...
async function viewHubNote(noteId) {
    try {
        ...
        `;
        showModal('📄 ' + note.title, html, `<button class="btn btn-outline" onclick="hubPrintNote()">🖨 Print</button> <button class="btn btn-outline" onclick="hubCopyNote(\`${safeContent}\`)">📋 Copy</button> <button class="btn btn-outline" onclick="downloadNote('${note.id}','txt')">⬇ Download</button>`);
    } catch (err) {
...
function hubDownloadText(content, title) {
    const text = content + '\n\nGenerated by NET Kenya - ' + new Date().toLocaleDateString();
    const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = (title || 'lesson-notes') + '.txt';
    a.click();
